...
exports.postTask = async (req, res) => {
  try {
    ...
    if (!dueDate || !priority) {
      return res.status(400).json({ status: false, msg: "Due date or priority not provided" });
    }

    const task = await Task.create({ user: req.user.id, description, dueDate, priority });
...
exports.putTask = async (req, res) => {
  try {
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, { description, dueDate, priority }, { new: true });
